...
You are working on branch ${task.branch}, which will be opened as a pull request when you finish. Make sure your work is actually committed: as you complete each piece of work, run \`git add\` and \`git commit\` with a clear, descriptive commit message explaining what changed and why. Do not finish with uncommitted changes — if \`git status\` shows anything pending, commit it before you stop.

Before you finish, run \`git fetch origin ${project.defaultBranch}\` and merge \`origin/${project.defaultBranch}\` into ${task.branch} (e.g. \`git merge origin/${project.defaultBranch}\`). If this produces merge conflicts, resolve them yourself, re-test if applicable, and commit the merge before stopping. The PR must be conflict-free against ${project.defaultBranch}.`;
